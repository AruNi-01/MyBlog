...
layout: Post
useHeaderImage: true
headerImage: https://aruni-01-github-io.oss-cn-beijing.aliyuncs.com/posts/nowcoder_img.avif
headerMask: rgba(40, 57, 101, .4)
catalog: true
---

仿牛客项目总结，从数据库设计到重点的功能实现，再到性能优化，另外自己添加了二级缓存。
...
- 单日 UV 的 key 为 uv:date，value 为 ip 地址；
- 区间 UV 的 key 为 uv:startDate:endDate，value 为 ip 地址；

对于 DUA：

- 单日 DUA的 key 为 dua:date，value 为 userId；
- 区间 DUA的 key 为 dua:startDate:endDate，value 为 userId；
